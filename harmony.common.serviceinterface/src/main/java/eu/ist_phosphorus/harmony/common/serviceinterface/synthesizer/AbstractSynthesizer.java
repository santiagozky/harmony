...
public abstract class AbstractSynthesizer implements Synthesizer {
    ...
    protected String convertType(final Class<?> returnType,
            final ClassInfo classInfo) {
        Class<?> returnType2 = returnType;
        final boolean isArray = returnType2.isArray();

        returnType2 = isArray ? ReflectUtils
                .getClassFromArrayClass(returnType2) : returnType2;

        // Use fully-qualified name if conflict exists
        if (classInfo.hasConflict(returnType2)) {
            return returnType2.getName();
        }
        return ReflectUtils.getShortName(returnType2) + (isArray ? "[]" : "");
    }
    ...
    protected String needsImport(final Class<?> className) {
        Class<?> className2 = className;
        if (className2.isPrimitive()) {
            return null;
        }
        if (className2.isArray()) {
            className2 = ReflectUtils.getClassFromArrayClass(className2);
        }
        if (className2.getName().startsWith("java.lang.")) {
            return null;
        }
        return className2.getName();
    }
