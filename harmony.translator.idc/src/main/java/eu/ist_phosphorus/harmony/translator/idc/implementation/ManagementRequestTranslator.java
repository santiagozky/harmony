...
public final class ManagementRequestTranslator extends HarmonyIdcTranslator {
    /**
     * HSI GetReservationsResponseType -> IDC ListReservationsResponse
     * 
     * @param arg
    ...
    /**
     * HSI GetReservationsType -> IDC ListReservations
     * 
     * @param arg
     * @return A list of reservations.
     */
    ...
    /**
     * HSI GetStatusResponseType -> IDC queryReservationResponse
     * 
     * @param arg
    ...
    /**
     * HSI GetStatusType -> IDC queryReservation
     * 
     * @param arg
    ...
    public GlobalReservationId convert(final GetStatusType arg) {
        ...
        id.setGri(this.reservationIdMapper.harmonyToIdc(arg.getReservationID(),
                false));
    ...
    /**
     * IDC ListReservations -> HSI GetReservationsType
     * 
     * @param arg
    ...
    /**
     * IDC ListReservationsResponse -> HSI GetReservationsResponseType
     * 
     * @param arg
     * @return The reservations.
     */
    public GetReservationsResponseType convert(final ListReply arg) {
        ...
        if (reply.getResDetails() == null) {
            ...
            for (final ResDetails detail : reply.getResDetails()) {
                ...
                try {
                    sourceEp.setEndpointId(this.endpointMapper.idcToHarmony(
                            idcSrc, true));
                } catch (final RuntimeException e) {
                    this.logger.info("Skipping " + idcSrc + "("
                            + e.getMessage() + ")");
                    continue;
                }
                ...
                try {
                    destEp.setEndpointId(this.endpointMapper.idcToHarmony(
                            idcDst, true));
                } catch (final RuntimeException e) {
                    this.logger.info("Skipping " + idcDst + "("
                            + e.getMessage() + ")");
                    continue;
                }
    ...
    /**
     * IDC queryReservation -> HSI GetStatusType
     * 
     * @param arg
    ...
    /**
     * IDC queryReservationResponse -> HSI GetStatusResponseType
     * 
     * @param arg
     * @return The status type.
    ...
    public GetStatusResponseType convert(final ResDetails arg)
        ...
        String status;
        if (IDCClient.STATUS_CANCELLED.equals(detail.getStatus())) {
            status = StatusType.CANCELLED_BY_USER.toString();
        } else if (IDCClient.STATUS_TEARDOWN.equals(detail.getStatus())) {
            status = StatusType.TEARDOWN_IN_PROGRESS.toString();
        } else if (IDCClient.STATUS_FAILED.equals(detail.getStatus())) {
            status = StatusType.CANCELLED_BY_SYSTEM.toString();
        } else if (IDCClient.STATUS_FINISHED.equals(detail.getStatus())) {
            status = StatusType.COMPLETED.toString();
        } else if (IDCClient.STATUS_INCREATE.equals(detail.getStatus())
                || IDCClient.STATUS_INSETUP.equals(detail.getStatus())
                || IDCClient.STATUS_ACCEPTED.equals(detail.getStatus())) {
        ...
        domainStatus.setDomain(Config.getString("hsiIDC", "domain.name"));
        ...
        try {
            ...
            throw new UnexpectedFaultException("The Status '" + status
                    + "' returned from IDC is not Supported by Harmony", e);
        }
        ...
        if (null != pathInfo.getLayer3Info()) {
            ...
            src.setEndpointId(this.endpointMapper.idcToHarmony(info
                    .getSrcHost()));
            dst.setEndpointId(this.endpointMapper.idcToHarmony(info
                    .getDestHost()));
        } else if (null != pathInfo.getLayer2Info()) {
            final Layer2Info info = pathInfo.getLayer2Info();

            // src.setEndpointId(this.endpointMapper.idcToHarmony(EndpointMapper
            // .getFullSrcAddress(info)));
            // dst.setEndpointId(this.endpointMapper.idcToHarmony(EndpointMapper
    ...
    public ListRequest convert(final IsAvailableType request) {
        ...
        final String[] status =
                {IDCClient.STATUS_ACTIVE, IDCClient.STATUS_INCREATE,
                        IDCClient.STATUS_INSETUP, IDCClient.STATUS_TEARDOWN};
        listReq.setResStatus(status);
        return listReq;
    }

    /**
     * Simulate an isAvailable response from an IDC.
     * 
     * @param listReservations
     *            The active IDC reservations.
     * @param request
    ...
    public IsAvailableResponseType simulateIsAvailable(
        ...
        availability.setMaxBW(Config.getInt("translatorIDC", "idc.maxBW"));
    ...
    private boolean containsEndpoint(final ListReply listReservations,
        ...
        for (final ResDetails reservation : listReservations.getResDetails()) {
            ...
            if (null != info) {
                final String target =
                        this.endpointMapper
                                .idcToHarmony(info.getDestEndpoint());
                final String source =
                        this.endpointMapper.idcToHarmony(info.getSrcEndpoint());
