...
package eu.ist_phosphorus.harmony.adapter.thin.implementation;
...
import eu.ist_phosphorus.gmpls.serviceinterface.databinding.jaxb.GetPathStatus;
import eu.ist_phosphorus.gmpls.serviceinterface.databinding.jaxb.GetPathStatusResponse;
import eu.ist_phosphorus.gmpls.serviceinterface.databinding.jaxb.GetPathStatusResponseType;
import eu.ist_phosphorus.gmpls.serviceinterface.databinding.jaxb.GetPathStatusType;
...
import eu.ist_phosphorus.harmony.adapter.thin.database.orm.GmplsReservation;
import eu.ist_phosphorus.harmony.adapter.thin.utils.Notifications;
import eu.ist_phosphorus.harmony.adapter.thin.webservice.ContextListener;
...
public final class StatusHandler {
	...
	final AJaxbSerializer jser = eu.ist_phosphorus.gmpls.serviceinterface.databinding.utils.JaxbSerializer
		.getInstance();
	...
	if (connections.isEmpty()) {
		...
		if (connection.convertStatus() == StatusType.ACTIVE) {
			...
			try {
			    ...
			    logger
				    .debug("connection wich was active on thin could not be found on gmpls-WS ");
    ...
    public static ServiceStatus getStatusOfService(final String reservationId,
	    final int serviceId, final boolean fromGmpls)
	    throws UnexpectedFaultException, InvalidReservationIDFaultException {
	return getStatusOfService(WebserviceUtils
		.convertReservationID(reservationId), serviceId, fromGmpls);
    }
	...
	GetReservationResponseType response = getReservation(
		getReservationRequest.getReservationID(), getReservationRequest
			.getServiceID());
	...
	List<GmplsReservation> gmplsReservations = DbManager
		.getReservationsInPeriod(getReservationsRequest
			.getPeriodStartTime(), getReservationsRequest
			.getPeriodEndTime());
	...
	for (GmplsReservation gmplsReservation : gmplsReservations) {
	    ...
	    grct.setReservation(getReservation(gmplsReservation
		    .getReservationId(), DbManager
    ...
    private GetReservationResponseType getReservation(
	    ...
	    throws UnexpectedFaultException, InvalidReservationIDFaultException {
	return getReservation(WebserviceUtils
		.convertReservationID(reservationId), serviceIds);
    }
}
