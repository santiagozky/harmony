...
import org.opennaas.extension.idb.da.thin.scheduler.JobManager;
...
public final class CancelHandler {
	...
	try {
	    ...
	    jser
		    .elementToObject(ContextListener
			    ...
			    .terminatePath(
				    (jser
					    .objectToElement(gmplsConnection
						    .getTerminatePathRequest(eu.ist_phosphorus.gmpls.serviceinterface.databinding.jaxb.StatusType.CANCELLED_BY_USER)))));
	...
	for (Integer serviceID : DbManager
	    ...
	    Notifications.publish(reservationId, StatusHandler
		    .getStatusOfService(reservationId, serviceID.intValue(),
			    false));
	}
	return responseType;
    }
