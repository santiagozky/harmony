 ...
 * @version $Id: TestReservationWS.java 4906 2009-08-20 15:46:29Z carlos.baez@i2cat.net $
 *
 */
...
public class TestReservationWS {
    ...
    public final void testCreateReservationWorkflow() throws SoapFault,
        ...
        IsAvailableResponseType availResult = this.client.isAvailable(Config
                .getString("test", "test.endpoint0.tna"), Config.getString(
                "test", "test.endpoint1.tna"), 100, 100, 100);
        for (ConnectionAvailabilityType a : availResult.getDetailedResult()) {
        ...
        CreateReservationResponseType createResult = this.client
        ...
        /* Get the status --------------------------------------------------- */
        GetStatusResponseType statusResult = this.client.getStatus(createResult
                .getReservationID());
        Assert.assertTrue("Should contain at least one reservation",
                statusResult.getServiceStatus().size() > 0);
        for (ServiceStatusType service : statusResult.getServiceStatus()) {
        ...
        CancelReservationResponseType cancelResult = this.client
                .cancelReservation(createResult.getReservationID());
        Assert.assertTrue("Cancel should succeed", cancelResult.isSuccess());
        /* ------------------------------------------------------------------ */

        /* Get the status --------------------------------------------------- */
        GetStatusResponseType statusResult2 = this.client
                .getStatus(createResult.getReservationID());
        for (ServiceStatusType service : statusResult2.getServiceStatus()) {
            Assert.assertTrue("Status should be active", service.getStatus()
    ...
    /**
     * Test method for
     * {@link eu.ist_phosphorus.harmony.adapter.arrm.webservice.ReservationWS#getReservations(org.w3c.dom.Element)}
     * .
     * 
     * @throws SoapFault
     * @throws DatatypeConfigurationException
    ...
    public final void testGetReservations() throws SoapFault,
        ...
        GetReservationsResponseType rsv = this.client.getReservations(10000);
