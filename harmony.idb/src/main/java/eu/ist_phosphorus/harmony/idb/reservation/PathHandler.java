...
public class PathHandler {
	...
	public PathHandler(List<Tuple<Endpoint, Endpoint>> pathParam, int minBw, int maxBw) {
		this.path = pathParam;
		this.bandwidthsOfEndpoints = new Hashtable<String, List<Integer>>(0);
		this.actualUsedBandwidthOfEndpoint = new Hashtable<String, Integer>(0);
	...
	private void initEndpointWithBandwidths() {
		for(Tuple<Endpoint, Endpoint> pathPart : this.path) {
			...
			this.bandwidthsOfEndpoints.put(end.getTNA(),
					   MalleableReservationHelpers
					   .getBwsForEndpoint(end.getTNA(),
							   			  this.globalMinBw,
							   			  this.globalMaxBw));
	 ...
	 */
	public Tuple<Boolean, Hashtable<String, Integer>> isBwAdaptingPossible(List<String> blockedEndpoints) {
		...
		for(Entry<String, List<Integer>> entry : this.bandwidthsOfEndpoints.entrySet()) {
			// sort the bandwidths
			List<Integer> l = MalleableReservationHelpers.sortBwsByStrategy(entry.getValue());
			// add list
			bws.put(entry.getKey(), l);
			// get pointer and save it
			pointer.put(entry.getKey(), l.indexOf(this.actualUsedBandwidthOfEndpoint.get(entry.getKey())));
		}
		...
		for(String tna : blockedEndpoints) {
			...
			if(newPointer == -1) {
				return new Tuple<Boolean, Hashtable<String, Integer>>(false, null);
			}
	...
	public List<String> getMinBwEndpoints() {
		...
		for(Tuple<Endpoint, Endpoint> pathPart : this.path) {
			...
			if(endBw == minBw) minBwEnds.add(endId);
			if(endBw < minBw) {
				minBwEnds.clear();
				minBwEnds.add(endId);
			}
	...
	public List<Tuple<Endpoint, Endpoint>> getPrunableEdges(List<String> blockedEndpoints,
															List<String> blockedDomains) {
		LinkedList<Tuple<Endpoint, Endpoint>> result = 
			new LinkedList<Tuple<Endpoint,Endpoint>>();
		...
		if(this.path != null) {
			...
			while(pointer < this.path.size()) {
				...
				if(blockedDomains.contains(firstPart.getFirstElement().getDomain().getName())
						|| blockedEndpoints.contains(firstPart.getFirstElement().getTNA())
						|| blockedEndpoints.contains(firstPart.getSecondElement().getTNA())) {
					result.add(firstPart);
				}

				// check for interdomain path-part
				if(pointer + 1 < path.size()) {
					Tuple<Endpoint, Endpoint> secondPart = path.get(pointer + 1);
					if(blockedEndpoints.contains(firstPart.getSecondElement().getTNA())
							|| blockedEndpoints.contains(secondPart.getFirstElement().getTNA())) {
						result.add(new Tuple<Endpoint, Endpoint>(
								firstPart.getSecondElement(),
								secondPart.getFirstElement()));
					}
				}
				pointer++;
			}
