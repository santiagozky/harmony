...
package eu.ist_phosphorus.harmony.idb.reservation;
...
import eu.ist_phosphorus.harmony.common.serviceinterface.databinding.jaxb.exceptions.PathNotFoundFaultException;
import eu.ist_phosphorus.harmony.common.utils.PerformanceLogLevel;
import eu.ist_phosphorus.harmony.common.utils.PhLogger;
import eu.ist_phosphorus.harmony.common.utils.Tuple;
import eu.ist_phosphorus.harmony.idb.database.hibernate.Connections;
import eu.ist_phosphorus.harmony.idb.database.hibernate.Domain;
import eu.ist_phosphorus.harmony.idb.database.hibernate.Endpoint;
import eu.ist_phosphorus.harmony.idb.database.hibernate.NrpsConnections;
import eu.ist_phosphorus.harmony.idb.database.hibernate.Reservation;
import eu.ist_phosphorus.harmony.idb.database.hibernate.Service;
import eu.ist_phosphorus.harmony.idb.exceptions.database.DatabaseException;
import eu.ist_phosphorus.harmony.idb.reservation.exceptions.InvalidConnectionIdException;
import eu.ist_phosphorus.harmony.idb.reservation.exceptions.InvalidServiceIdException;
import eu.ist_phosphorus.harmony.idb.reservation.handler.MalleableReservationHelpers;
import eu.ist_phosphorus.harmony.idb.reservation.pathcomputer.IPathComputer;
import eu.ist_phosphorus.harmony.idb.reservation.pathcomputer.MockPathComputerScen1;
import eu.ist_phosphorus.harmony.idb.reservation.pathcomputer.PathComputer;
...
public class PathFinderNG {
     ...
     */
    public PathFinderNG(final Reservation reservation, final IManager manager, boolean subdomainRestriction, boolean malleableParam)
        ...
        for (final Service service : this.reservation.getServices().values()) {
            try {
                ...
                for (final Connections connection : service.getConnections()
                    ...
                    final List<Tuple<Endpoint, Endpoint>> path = 
                    		getActualPath(service.getServiceId(), connection.getConnectionId());
    ...
    public void calculateNrpsConnections(int serviceId, int connId) throws PathNotFoundFaultException {
    ...
    public HashMap<Domain, Reservation> getReservations()
        ...
        if (!this.processed) {
        	if (isMultidomain()) {
            	...
            	this.outputReservations = getAvailabilityRequests().getSecondElement();
        	}
        }
    ...
    protected void initPathComputer() throws EndpointNotFoundFaultException,
    	...
    	try {
			Constructor<? extends IPathComputer> c = pcClass.getConstructor(boolean.class);
			this.pathComputer = c.newInstance(this.subdomainRestriction);
		} catch (NoSuchMethodException e) {
	        try {
	            this.pathComputer = pcClass.newInstance();
	        } catch (final InstantiationException e1) {
	            throw new RuntimeException("error initializing the path computer", e1);
	        } catch (final IllegalAccessException e1) {
	            throw new RuntimeException("error initializing the path computer", e1);
	        }
		} catch (SecurityException e) {
            throw new RuntimeException("error initializing the path computer", e);
        ...
        for (final Service service : this.reservation.getServices().values()) {
            ...
            try {
                this.pathComputer
                        .addService(service.getStartTime().getTime(), service
                                .getStartTime().getTime()
                                + service.getDuration(), service.getServiceId());
            } catch (final InvalidServiceIdException e) {
            ...
            for (final Connections connection : service.getConnections()
                ...
                try {
                	...
                	final Endpoint target = connection.getEndpoints().iterator().next();
                	final Domain sourceDomain = source.getDomain();
                	final Domain targetDomain = target.getDomain();
                	/*if (! sourceDomain.getName().equals(targetDomain.getName())) {
                		this.interdomain = true;
                	}*/
                	this.endDomains.add(sourceDomain);
                	this.endDomains.add(targetDomain);
                    this.pathComputer.addConnection(source, target, service
							.getServiceId(), connection.getConnectionId());
                } catch (final InvalidServiceIdException e) {
    ...
    protected void process() throws DatabaseException, SoapFault {
        ...
        while (!availablePathFound) {
            ...
            try {
            	final Tuple<Hashtable<Domain, IsAvailableType>, HashMap<Domain, Reservation>> 
            										result = getAvailabilityRequests();
            ...
            for (final Domain domain : nrpsResponses.keySet()) {
                ...
                for (final ConnectionAvailabilityType ca : nrpsResponse
                    ...
                    final Tuple<Integer, Integer> id = new Tuple<Integer, Integer>(
                            new Integer(ca.getServiceID()), new Integer(ca
                                    .getConnectionID()));
                    ...
                    if (availability != AvailabilityCodeType.AVAILABLE) {
                        ...
                        if (availability != AvailabilityCodeType.AVAILABILITY_NOT_CHECKED) {
                            ...
                            s = nrpsReservationTable.get(domain)
                                    .getServices().get(
                                    		Integer.valueOf(ca.getServiceID()));
                            if (s != null) {
                        ...
                        if (availability == AvailabilityCodeType.ENDPOINT_NOT_AVAILABLE) {
                            if (s == null) {
                                ...
                                nrpsReservationTable.get(domain)
                                        .getServices().get(
                                                Integer.valueOf(ca
                                                        .getServiceID()));
                            }
                ...
                if (!(available || pruned)) {
                    ...
                    for (final ConnectionAvailabilityType ca : nrpsResponse
                        ...
                        if (ca.getAvailability() == AvailabilityCodeType.AVAILABILITY_NOT_CHECKED) {
                            final Service s = nrpsReservationTable.get(
                                    domain).getServices().get(
                                    Integer.valueOf(ca.getServiceID()));
                            this.pruneStuff(s, ca.getConnectionID(),
    ...
    private void processMalleable() throws SoapFault, DatabaseException {
        ...
        MalleableReservationHelpers malleableHelpers = MalleableReservationHelpers.getInstance();
    	...
    	// initial AvailabilityRequest-creation and converting from malleable to fixed
    		...
    		for(Connections conn : service.getConnections().values()) {
    			...
    			while(!reservationFound) {
        			...
        			while(computeNewPath) {
    	    			try {
							calculateNrpsConnections(service.getServiceId(), conn.getConnectionId());
		    				...
		    				String exMessage = "no feasible path available "
								   ...
								   + "[" + conn.getStartpoint().getTNA() + "] to "
								   + "[" + ((Endpoint)conn
										   			.getEndpoints().toArray()[0]).getTNA() 
								   + "] !";
		    				malleableLogger.error(exMessage);
				            this.pathNotFound = new PathNotFoundFaultException(
				                    "No path could be found: " + e.getMessage() + "(info:"
				                            + this.logMsg + ")",
				                    e);
				            return;
				        }
    	        		...
    	        		// save path
    	    			try {
							actualPh = new PathHandler(
									pathComputer.getPath(service.getServiceId(), conn.getConnectionId()),
									conn.getMinBandwidth(), conn.getMaxBandwidth());
		        			String pathLog = "computed path: ";
		        			for(Tuple<Endpoint, Endpoint> pathPart : actualPh.getPath()) {
		        				pathLog += "[" 
		        			...
		        			malleableLogger.error("wrong service-ID for pathfinder!");
		        		} catch (InvalidConnectionIdException e1) {
		        			malleableLogger.error("wrong connection-ID for pathfinder!");
		        		}
    	
    	    			// create advanced availability requests
    	    			actualRc = new FixedAvailabilityRequestsConverter(actualPh);
    	    	    	
    	    			actualRc.createRequestsFromMalleable(this.reservation.getReservationsForDomains(),
    	    										   service.getServiceId(),
    	    										   conn.getConnectionId());
    	    			
    	        		// validate availability requests
    	    			if(!actualRc.validateRequests()) {
    	        			malleableLogger.debug("Duration too long! Trying next path!");
    	    				// the duration is too long, remove minBw-edges and compute new path
    	    				List<String> minBwEnds = actualPh.getMinBwEndpoints();
    	
    	    				Endpoint precedentEndpoint = null;
    	    				for(Tuple<Endpoint, Endpoint> pathPart : actualPh.getPath()) {
    	    					// intraDomain-edge
    	    					if(minBwEnds.contains(pathPart.getFirstElement().getTNA())
    	    					   || minBwEnds.contains(pathPart.getSecondElement().getTNA())) {
    	 							try {
    	 		            			malleableLogger.debug("pruning edge [" + pathPart.getFirstElement().getTNA() + " -> " + pathPart.getSecondElement().getTNA() + "]");
										this.pruneExactOneEdge(service, 
												 conn.getConnectionId(),
												 pathPart.getFirstElement(),
												 pathPart.getSecondElement());
									} catch (EndpointNotFoundFaultException e) {
										malleableLogger.error("wrong endpoint for pathfinder!");
									} catch (InvalidServiceIdException e1) {
					        			malleableLogger.error("wrong service-ID for pathfinder!");
					        		} catch (InvalidConnectionIdException e1) {
					        			malleableLogger.error("wrong connection-ID for pathfinder!");
    	        				...
    	        				if(precedentEndpoint != null) {
    	        					if(minBwEnds.contains(precedentEndpoint.getTNA())
    	        					   || minBwEnds.contains(pathPart.getFirstElement().getTNA())) {
    	        						try {
        	 		            			malleableLogger.debug("pruning edge [" + precedentEndpoint.getTNA() + " -> " + pathPart.getFirstElement().getTNA() + "]");
											this.pruneExactOneEdge(service, 
																	 conn.getConnectionId(),
																	 precedentEndpoint,
																	 pathPart.getFirstElement());
            		...
            		for(Domain d : actualRc.getRequests().keySet()) {
            			for(ServiceConstraintType s : actualRc.getRequests().get(d).getService()) {
            				for(ConnectionConstraintType c : s.getConnections()) {
            					for(EndpointType e : c.getTarget()) {
            						malleableLogger.debug(
            								"request for domain: " + d.getName() + "\n"
            								+ "for connection from [" + c.getSource().getEndpointId() 
            								+ "] to [" + e.getEndpointId() + "]\n"
            								+ "start: " 
            								+ s.getFixedReservationConstraints().getStartTime()
            								+ " duration: " 
            								+ s.getFixedReservationConstraints().getDuration()
            								+ " bandwidth: "
            								+ actualPh.getActualBwOfEndpoint(c.getSource().getEndpointId())
            								+ " | "
            								+ actualPh.getActualBwOfEndpoint(e.getEndpointId()));
            					}
            				}
            			}
            		}
		    		...
		    		Hashtable<Domain, IsAvailableResponseType> nrpsResponses =
		   								this.nrpsManager.isAvailable(actualRc.getRequests());

		    		int availabilitySum = 0;

		    		long altOffset = 0l;
		    		long actualEndTimeMillis = actualRc.getActualStartTime().getTime() + (actualRc.getActualDuration() * 1000l);

		    		// list of blocked endpoints found in one timeslot evaluation for a specific service
		    		List<String> blockedEndpoints = new ArrayList<String>();

		    		// list of blocked domains found in one timeslot evaluation for a specific service
		    		...
		    		for(Domain d : nrpsResponses.keySet()) {
		    			IsAvailableResponseType nrpsResponse = nrpsResponses.get(d);
		    			for(ConnectionAvailabilityType cat : nrpsResponse.getDetailedResult()) {
		    				AvailabilityCodeType av = cat.getAvailability();
		    				long altOffsetD = 0l;
		    				if ((av == AvailabilityCodeType.ENDPOINT_NOT_AVAILABLE) || (av == AvailabilityCodeType.PATH_NOT_AVAILABLE)) {
		    					if (nrpsResponse.isSetAlternativeStartTimeOffset()) {
		    						altOffsetD = nrpsResponse.getAlternativeStartTimeOffset();
		    					}
		    					if (altOffsetD <= 0l) { // erroneous replies may contain negative values
		    						altOffsetD = 3600;
		    					}
		    				}
		    				final boolean prune;
		    				if (altOffsetD > 0l) {
		    					long offsetCheck = actualEndTimeMillis + (altOffsetD * 1000l);
		    					prune = (offsetCheck > actualRc.getGlobalDeadline().getTime());
		    					malleableLogger.debug("offsetCheck: " + actualEndTimeMillis + " + " + (altOffsetD * 1000l) + " <?> " + actualRc.getGlobalDeadline().getTime());
		    					malleableLogger.debug("offsetCheck human-readable: " + actualRc.getActualStartTime() + " + " + actualRc.getActualDuration() + "s + " + altOffsetD + "s <?> " + actualRc.getGlobalDeadline());
		    				} else {
		    					prune = false;
		    				}
//		    				Service domainService = null;
//		    				if (prune) {
//		    					domainService = result.getSecondElement().get(d).getService(cat.getServiceID());
//		    					if (domainService == null) {
//		    						String ss = "";
//		    						for (Integer i : result.getSecondElement().get(d).getServices().keySet()) {
//		    							ss += " " + i;
//		    						}
//		    						malleableLogger.error("cannot retrieve service with ID " + cat.getServiceID() + " for domain " + d.getName() + ", available IDs: " + ss);
//		    					}
//		    				}
		    	            ...
		    	            if (av == AvailabilityCodeType.ENDPOINT_NOT_AVAILABLE) {
		    	                ...
		    	                if (blockedEndpoints.size() == 0) { // UCLP adapter hack
		    						...
		    						this.pruneStuff(service, cat.getConnectionID(), av, cat.getBlockedEndpoints());
		    	                }
		    	            }
		    				if (av == AvailabilityCodeType.PATH_NOT_AVAILABLE) {
		    					...
		    					if (prune) {
		    						this.pruneStuff(service, cat.getConnectionID(), av, null);
		    					}
		    				}

		    	            availabilitySum = Math.max(availabilitySum, av.ordinal());

		    				String logMsg = "availability " + d.getName() + " for service: " 
		    					+ cat.getServiceID() + " and connection: "
		    					+ cat.getConnectionID() + ":\n"
		    					+ cat.getAvailability() + "\n"
		    					+ "offset: " + nrpsResponses.get(d).getAlternativeStartTimeOffset() 
		    					+ "\n blocked: ";
					...
					if(availabilitySum > 0) {
						...
						XMLGregorianCalendar newStartTime = malleableHelpers.computeNewStartTime(
														nrpsResponses,
						...
						if(newStartTime != null) {
							...
							if(bwPossible.getFirstElement()) {
								...
								// -> use the same path, but adapt the requests to new bandwidth
				    			// -> new durations have to be computed and the availability-requests
				    			// on the same path should start from the initial startTime again
								actualRc.adaptToNewBws(bwPossible.getSecondElement());

					    		//set the startTime back
					    		actualRc.shiftRequests(actualRc.getGlobalStarttime());
			    				// no new path computing
			    				computeNewPath = false;
							} else {
			    				// found no feasible startTime on that path and the BW could not be 
								// adapted -> prune edges of blocked endpoints, edges of not-available
								// domains and try to find a new path

								List<Tuple<Endpoint, Endpoint>> prunableEdges =
									actualPh.getPrunableEdges(blockedEndpoints, blockedDomains);
								...
								for(Tuple<Endpoint, Endpoint> edge : prunableEdges) {
					    			...
					    			try {
										...
										malleableLogger.error("This should not happen!");
										e.printStackTrace();
		    			...
		    			for(Entry<Domain, Reservation> entry : actualRc.getActualReservations().entrySet()) {
...
//                        String.valueOf(connectionId));
                ...
                if (c == null) {
                	throw new RuntimeException("connection id " + connectionId + " does not exist!");
                }
     ...
     */
    public void pruneExactOneEdge (final Service service, final int connectionId,
            final Endpoint start, final Endpoint target)
            throws EndpointNotFoundFaultException, InvalidServiceIdException, InvalidConnectionIdException {
     ...
     */
    public Tuple<Hashtable<Domain, IsAvailableType>, HashMap<Domain, Reservation>> 
    									getAvailabilityRequests() throws DatabaseException, PathNotFoundFaultException {
    	Tuple<Hashtable<Domain, IsAvailableType>, HashMap<Domain, Reservation>> result;
    	Hashtable<Domain, IsAvailableType> nrpsRequests = 
    											new Hashtable<Domain, IsAvailableType>();
        HashMap<Domain, Reservation> nrpsReservationTable = 
    ...
    /**
     * get the actual computed path from the pathComputer-object of this pathFinder
     * 
     * @param serviceId
     * @param connectionId
     * @throws PathNotFoundFaultException
     * @throws InvalidServiceIdException
     */
    public List<Tuple<Endpoint, Endpoint>> getActualPath(int serviceId, int connectionId) 
     ...
     * @throws InvalidServiceIdException
     */
    public void computePaths(int serviceId)
