...
public class TestDomSupportedAdaptionRelation extends DatabaseTest {
    ...
    public static void tearDownAfterClass() throws DatabaseException {
        ...
        Assert.assertNull("reference domain should be deleted",
                Domain.load(TestDomSupportedAdaptionRelation.domain
                        .getName()));
    }
	...
	public void setUpBeforeEveryTest() {
		...
		this.adaption = new DomSupportedAdaption(TestDomSupportedAdaptionRelation.domain,
    ...
    public void testSaveLoadEditDeleteRelationTuple() throws DatabaseException {
        ...
        Assert.assertEquals(
                "persisted and loaded adaption should be equal",
        ...
        final DomSupportedAdaption editedAd = DomSupportedAdaption.load(loadedAd
                .getPK_Adaption());
        Assert.assertNotSame("edited adaption should be different",
                copyAd, editedAd);

        // delete
        loadedAd.getDomain().getSupportedAdaptions().remove(loadedAd);
        loadedAd.delete();
        Assert.assertNull("test adaption should be deleted", DomSupportedAdaption
                .load(pk));
    }
    ...
    public void testLoadAll() throws DatabaseException {
    	...
    	Assert.assertTrue("first adaption should be existent", setAds.contains(this.adaption));
    	Assert.assertTrue("second adaption should be existent", setAds.contains(secondAd));

        // delete
        this.adaption.getDomain().getSupportedAdaptions().clear();
        this.adaption.delete();
        secondAd.delete();
        Assert.assertNull("first adaption should be deleted", DomSupportedAdaption
                .load(this.adaption.getPK_Adaption()));
        Assert.assertNull("second adaption should be deleted", DomSupportedAdaption
                .load(secondAd.getPK_Adaption()));
    }
}
