 ...
 */
public final class MalleableReservationHandler extends CommonReservationHandler {

    public final static class HelperStatus {
    ...
    public static MalleableReservationHandler getInstance() {
        if (MalleableReservationHandler.selfInstance == null) {
            MalleableReservationHandler.selfInstance = new MalleableReservationHandler();
        }
        return MalleableReservationHandler.selfInstance;
    }

    /** logger instance. */
    private final Logger logger = PhLogger
    ...
    /**
     * 
     * @see MalleableReservationHandler#runRequest(CancelReservationType)
     */
    @Override
    ...
    public IsAvailableResponseType isAvailable(final IsAvailableType request) {
        ...
        final ConnectionAvailabilityType cat = new ConnectionAvailabilityType();
        ...
        final AvailabilityCodeType availabilityLink = this.checkAvailability(
                sourceTNA, destinationTNA);
        this.logger.debug("availability of link " + sourceTNA + ":"
                + destinationTNA + " : " + availabilityLink);

        // get the availabilities of the endpoints
        final AvailabilityCodeType availabilitySource = this
                .checkAvailability(sourceTNA);
        this.logger.debug("availability of source " + sourceTNA + ": "
                + availabilitySource);
        final AvailabilityCodeType availabilityDest = this
                .checkAvailability(destinationTNA);
        this.logger.debug("availability of destination " + destinationTNA
                + ": " + availabilityDest);

        final int availabilitySum = Math.max(availabilityLink.ordinal(), Math
                .max(availabilitySource.ordinal(), availabilityDest.ordinal()));
        ...
        if (availabilitySum > 0) {
            ...
            if (availabilityLink
                ...
                offsetLink = this.linkStatus.get(
                        sourceTNA + ":" + destinationTNA).getStartTimeOffset();
            }
            ...
            if (availabilitySource
                ...
                offsetSource = this.endpointStatus.get(sourceTNA)
                        .getStartTimeOffset();
            }
            ...
            if (availabilityDest
                ...
                offsetDestination = this.endpointStatus.get(destinationTNA)
                        .getStartTimeOffset();
            }
