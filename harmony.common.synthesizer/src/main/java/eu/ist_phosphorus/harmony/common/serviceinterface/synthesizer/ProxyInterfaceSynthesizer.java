...
package eu.ist_phosphorus.harmony.common.serviceinterface.synthesizer;
...
public class ProxyInterfaceSynthesizer extends AbstractSynthesizer {
	...
	protected final static Map<Class<? extends AbstractResourceClient>, Set<String>> _clientCapabilitiesMap;
	...
	static {
		_clientCapabilitiesMap = new HashMap<Class<? extends AbstractResourceClient>, Set<String>>();
	...
	private Map<String, String>[] _capabilityMaps;
	...
	protected void generateCode(final Document wsdl,
			final Map<String, String> capabilityMap,
		...
		for (final Object name : capabilityMap.values()) {
			final Capability capability = (Capability) name;
		...
		files.put(this.createFileName(className),
				this.generateCombinedCode(classInfo));
	}
	...
	private Class<?> getBaseClientClass(final Map<String, String> capabilityMap) {
	...
	protected void loadParameters(final ConfigurationData data) {
		this._generateCustomHeaders = ((Boolean) data
				...
				.booleanValue();
		this._capabilityMaps = (Map<String, String>[]) data
		...
		this._filesMaps = (Map<String, String>[]) data
		...
		if (this._filesMaps == null) {
			...
			data.addParameter(ConfigurationData.FILES_MAP_LIST, this._filesMaps);
		}
	}
	...
	private boolean needsGeneratedCode(final Class<?> clientClass,
		...
		final Set<?> set = ProxyInterfaceSynthesizer._clientCapabilitiesMap
				.get(clazz);
		if (set.contains(uri)
				|| ProxyInterfaceSynthesizer._ignoredCapabilitySet
						.contains(uri)) {
	...
	public ConfigurationData synthesize(final ConfigurationData data)
		...
		for (int i = 0; i < this._capabilityMaps.length; i++) {
			if (this._filesMaps[i] == null) {
				this._filesMaps[i] = new HashMap<String, String>();
			}
	...
	protected void updateMethods(final ClassInfo classInfo,
		...
		while (i.hasNext()) {
			...
			if (property.isAppendable()) {
				this.generatePropertyInsert(property, classInfo,
						this.getIndex(), code);
			}

			if (property.isMutable()) {
				this.generatePropertyUpdate(property, classInfo,
						this.getIndex(), code);
				this.generatePropertyDelete(property, classInfo,
						this.getIndex(), code);
			}
	...
	private void updateProperties(final ClassInfo classInfo,
			final StringBuffer code) {
		final Iterator<?> i = classInfo.getCapability().getProperties()
				.iterator();
